...
import { jsPDF } from 'jspdf'; // Ensure to import jsPDF
import 'jspdf-autotable'; // Import autotable for jsPDF if needed
...
const Juniors = () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            {rankings.slice(0, showMore ? rankings.length : 5).map((player, index) => (
                <div key={index} className="shadow-lg rounded-lg p-4 text-center border bg-white">
                    ...
                    <button onClick={() => generatePDF(player.id)} className="mt-2 bg-green-500 text-white px-4 py-2 rounded">Generate PDF</button>
                </div>
            ))}
        </div>
    );
    ...
    const renderEditForm = () => (
        <form onSubmit={handleUpdatePlayer} className="p-4 max-w-md mx-auto bg-white shadow-lg rounded">
            ...
            <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded">Update Player</button>
            <button type="button" onClick={() => setEditMode(false)} className="bg-gray-500 text-white px-4 py-2 rounded ml-2">Cancel</button>
        </form>
    );
    ...
    return (
        <div>
            ...
            <h1 className="text-3xl font-bold mb-4">Junior Players Rankings</h1>
            {editMode ? renderEditForm() : renderPlayerCards()}
            <button onClick={toggleShowMore} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded">
